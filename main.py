...
def highlight_risks(row):
    ...
    # Get the actual column names from the row's index
    col_names = row.index
    ...
    if platform_license_col is not None:
        license_value = row[platform_license_col]
        if license_value in [VALUE_TRANSLATIONS["NO-LICENSE"]["en"], 
                            VALUE_TRANSLATIONS["SUSPENDED"]["en"],
                            VALUE_TRANSLATIONS["NO-LICENSE"]["zh"], 
                            VALUE_TRANSLATIONS["SUSPENDED"]["zh"]]:
            styles[platform_license_col] = 'background-color: #FFCCCB'
        
    # Capital adequacy
    if capital_ratio_col is not None and row[capital_ratio_col] < 8:
        styles[capital_ratio_col] = 'background-color: #FFD580'
        
    # Predatory lending
    if (interest_rate_col is not None and credit_score_col is not None and
        row[interest_rate_col] > 24 and row[credit_score_col] < 650):
    ...
    if related_party_col is not None:
        party_value = row[related_party_col]
        if party_value in [VALUE_TRANSLATIONS["Yes"]["en"], VALUE_TRANSLATIONS["Yes"]["zh"]]:
            styles[related_party_col] = 'background-color: #90EE90'
        
    # KYC issues
    if kyc_status_col is not None:
        kyc_value = row[kyc_status_col]
        if kyc_value not in [VALUE_TRANSLATIONS["Verified"]["en"], VALUE_TRANSLATIONS["Verified"]["zh"]]:
...
# Now translate the column names for display
display_df.columns = [COLUMN_TRANSLATIONS.get(col, {}).get(lang_key, col) 
                     for col in display_df.columns]
pply(highlight_risks, axis=1), height=400)
